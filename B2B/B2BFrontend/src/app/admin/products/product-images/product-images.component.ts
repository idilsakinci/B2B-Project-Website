...
export class ProductImagesComponent implements OnInit {
  ...
  getList() {
    this.productImagesService.getList(this.productId).subscribe((res: any) => {
  ...
  uploadImages() {
    ...
    this.productImagesService.add(formData).subscribe((res: any) => {
      this.getList();
    }, (err) => {
      this.errorService.errorHandler(err);
    });
  ...
  deleteImage(productImage: ProductImageModel) {
    this.productImagesService.delete(productImage).subscribe((res) => {
      this.getList();
    }, (err) => {
      this.errorService.errorHandler(err);
    });
  ...
  setMainImage(id: number) {
    this.productImagesService.setMainImage(id).subscribe((res: any) => {
      this.getList();
    }, (err) => {
      this.errorService.errorHandler(err);
    });
