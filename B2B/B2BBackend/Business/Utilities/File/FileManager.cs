...
namespace Business.Concrete
{
    public class FileManager : IFileService
    {
        ...
        public string FileSaveToFtp(IFormFile file)
        {
            ...
            fileFormat = fileFormat.ToLower();
            string fileName = Guid.NewGuid().ToString() + fileFormat;
            FtpWebRequest request = (FtpWebRequest)WebRequest.Create("Your FTP Address will be written" + fileName);
        ...
        public void FileDeleteToFtp(string path)
        {
            try
            {
                FtpWebRequest request = (FtpWebRequest)WebRequest.Create("ftp address" + path);
                request.Credentials = new NetworkCredential("Username", "Password");
